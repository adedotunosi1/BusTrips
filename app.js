...
const busTripsRouter = require("./src/routes");
...
const allowedOrigins = ["https://yum-run.vercel.app/", "http://localhost:5173", "http://localhost:4000", "http://localhost:3000", "https://afrimove.vercel.app", "https://afrimove.vercel.app/"];
...
// app.use(makeUser);
app.use(makeUsers)
app.use("/api/v1/", busTripsRouter);
app.use("/api/v1", (req, res, next) => {
  res.send({ msg: `Yes!... Welcome to BusTrips API` });
  next();
});
app.use(serverRequests);
app.get("/", (req, res, next) => {
    res.send(`
    <h2> BusTrips Server Live! </h2>
    `)
    next();
})
